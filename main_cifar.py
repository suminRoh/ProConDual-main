...
parser.add_argument('--dataset', default='cifar-10-lt', choices=['cifar-10-lt','cifar-100-lt'])
parser.add_argument('--arch', default='resnet34', choices=['resnet50', 'resnet34'])
...
parser.add_argument('--feat_dim', default=512, type=int, help='feature dimension of mlp head')
...
parser.add_argument('--root_log', type=str, default='/data/ProConDual/log')
parser.add_argument('--root_save', type=str, default='/data/ProConDual/save')

def main():
    args = parser.parse_args()
    args.store_name = '_'.join(
        [args.dataset, args.arch, 'bs', str(args.batch_size),'lr', str(args.lr),'imb',str(args.imb_factor),'dim',str(args.feat_dim)])
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.arch == 'resnet50': 
        ...
        model = resnet.ProConDual(name='resnet18', num_classes=args.num_classes, feat_dim=args.feat_dim,
                                 use_norm=args.use_norm)
    elif args.arch == 'resnet34':  
        model = resnet.ProConDual(name='resnet34', num_classes=args.num_classes, feat_dim=args.feat_dim,
                                 use_norm=args.use_norm)
    ...
    if args.dataset=='cifar-10-lt':    
        train_dataset = IMBALANCECIFAR10(root='./dataset/cifar-10-lt', train=True,
                                    download=True, transform=transform_train,imb_factor=args.imb_factor)
        
        val_dataset = IMBALANCECIFAR10(root='./dataset/cifar-10-lt', train=False,
                                    download=False, transform=val_transform,imb_factor=args.imb_factor)    
    
    if args.dataset=='cifar-100-lt':    
        train_dataset = IMBALANCECIFAR100(root='./dataset/cifar-100-lt', train=True,
                                    download=True, transform=transform_train,imb_factor=args.imb_factor)
        
        val_dataset = IMBALANCECIFAR100(root='./dataset/cifar-100-lt', train=False,
                                    download=True, transform=val_transform,imb_factor=args.imb_factor)    
    


    cls_num_list = train_dataset.num_per_cls_dict  
    cls_num_list = list(cls_num_list.values())
    ...
    if args.reload:
        if args.dataset=='cifar-10-lt':        
            test_dataset = IMBALANCECIFAR10(root='./dataset/cifar-10-lt', train=False,
                                        download=False, transform=val_transform,imb_factor=args.imb_factor)    
        
        if args.dataset=='cifar-100-lt':    
            test_dataset = IMBALANCECIFAR100(root='./dataset/cifar-100-lt', train=False,
                                        download=True, transform=val_transform,imb_factor=args.imb_factor)    
        
        test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=args.batch_size, shuffle=False,
        num_workers=args.workers, pin_memory=True)
        acc1= validate(train_loader, test_loader, model, criterion_lc, 1, args, tf_writer)
